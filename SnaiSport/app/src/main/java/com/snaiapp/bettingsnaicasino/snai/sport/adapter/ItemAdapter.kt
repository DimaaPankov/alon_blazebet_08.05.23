...
val savedSportsAppReviews = BooleanArray(20) { false }

var isPlayerReview = true
var sports =     arrayListOf(
...
var sportsAppReviews:ArrayList<String> = arrayListOf<String>()

fun setAppsList(){
    if(isPlayerReview) { sportsAppReviews =  sports}
    else{
        var listSaved = arrayListOf<String>()
        for(i in 0..savedSportsAppReviews.size-1){
            if(savedSportsAppReviews[i]){
                listSaved.add(sports[i])
            }
        }
       sportsAppReviews =  listSaved
    }
}
...
class Adapter() : RecyclerView.Adapter<Adapter.ViewHolder>() {
    ...
    inner class ViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
        ...
        val ivhert: ImageView = itemView.findViewById(R.id.ivheart)
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        setAppsList()
        holder.textView.text = sportsAppReviews[position]
        if(!isPlayerReview){holder.ivhert.isVisible = true}
        else{
                if(savedSportsAppReviews[position]){
                    holder.ivhert.isVisible = true
                }
        }
        holder.ivhertempty.setOnClickListener{
            if(!savedSportsAppReviews[position]){
             holder.ivhert.isVisible = true
                savedSportsAppReviews[position] = true
            }else{
                holder.ivhert.isVisible = false
                savedSportsAppReviews[position] = false
            }
        }
    }

    override fun getItemCount(): Int {
        return sportsAppReviews.size
    }
}
