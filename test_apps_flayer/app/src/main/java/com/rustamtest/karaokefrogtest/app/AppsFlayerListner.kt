...
import com.rustamtest.karaokefrogtest.black.advertising_ID
import com.rustamtest.karaokefrogtest.black.doneUrl
import com.rustamtest.karaokefrogtest.black.fireBaseState
import com.rustamtest.karaokefrogtest.black.mainUrl
import com.rustamtest.karaokefrogtest.parseSubs
...
object AppsFlayerListner : AppsFlyerConversionListener {
    ...
    var actionToOffer: () -> Unit = {}
    var actionToGame: () -> Unit = {}
    ...
    override fun onConversionDataSuccess(data: MutableMap<String, Any>?) {
        ...
        if (campaign == null && !fireBaseState) {
            actionToGame()

        } else if (campaign == null && fireBaseState) {
            doneUrl = "$mainUrl?$appsFlyerID&$appsFlyerID"
            actionToOffer
        } else {
            val subs = parseSubs(campaign!!, appsFlyerID, advertising_ID)
            doneUrl = mainUrl.take(mainUrl.indexOf('?') + 1)
                .let { url ->
                    url + subs?.map { it.key + "=" + it.value }
