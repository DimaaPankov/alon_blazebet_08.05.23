...
object AppsFlayerListner : AppsFlyerConversionListener {
    ...
    override fun onConversionDataSuccess(data: MutableMap<String, Any>?) {
        ...
        af_id = URLEncoder.encode((data?.get("adset") ?: "null").toString(), "utf-8")
        ...
        is_first_launch =
            URLEncoder.encode((data?.get("is_first_launch") ?: "null").toString(), "utf-8")
        ...
        fb.fetchAndActivate()
            .addOnCompleteListener { status ->
                if (status.isSuccessful) {
                    ...
                    if (url.isNotBlank()) {
                        finalUrl = setUrlConvertation(
                            if (!(url.last().toString() == "?")) url + "?" else url
                        )
                        Log.d("URLf", finalUrl)
                        ...
                        or ModelPhone.getPhoneModel()!!.contains("Google", ignoreCase = true))*/
                            ...
                            var isOrgabicAF = if (af_status == "Organic") {
                                true
                            } else {
                                false
                            }
